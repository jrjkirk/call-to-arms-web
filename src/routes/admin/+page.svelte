...
{:else}
    ...
    <div class="admin-shell" in:fly={{ y: 24, duration: 550, easing: cubicOut }}>
        <aside class="admin-sidebar">
            ...
            {#if adminMe.community_discord_url}
                <a class="admin-discord-link" href={adminMe.community_discord_url} target="_blank" rel="noopener noreferrer">
                    <svg class="admin-discord-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    ...
    {#if DISCORD_GATE_ENABLED && activeNav === 'discord' && adminMe.is_super_admin}
        ...
        <section class="admin-section">
            <DiscordGatePanel
                {gate}
                bind:guildInput={gateGuildInput}
                saving={gateSaving}
                error={gateError}
                message={gateMessage}
                copied={gateInviteCopied}
                onSave={saveDiscordGate}
                onCopy={copyBotInvite}
            />
        </section>
            </div>
        </div>
...
<style>
    ...
    /* Community/support Discord link — pinned in the admin sidebar, in Discord
       blurple so it's easy to spot from any section. */
    .admin-discord-link {
        display: inline-flex;
        align-items: center;
        gap: 0.5rem;
    ...
    .admin-discord-link:hover { background: #4752c4; transform: translateY(-1px); }
    .admin-discord-icon { width: 18px; height: 18px; flex: 0 0 auto; }
